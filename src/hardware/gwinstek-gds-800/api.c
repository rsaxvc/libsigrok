...
static struct sr_dev_inst *probe_device(struct sr_scpi_dev_inst *scpi, struct sr_dev_driver *driver_info)
{
	...
	for (i = 0; i < devc->num_acq_channel; i++) {
		...
		if (gwinstek_gds_800_fetch_volts_per_div(sdi->conn, i, &devc->vdivs[i]) != SR_OK)
			return NULL;

		channel_name = g_strdup_printf("CH%d", i + 1);

		devc->analog_groups[i] = sr_channel_group_new(sdi, channel_name, NULL);
		ch = sr_channel_new(sdi, i, SR_CHANNEL_ANALOG, channel_enabled, channel_name);
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_NUM_VDIV:
		*data = g_variant_new_int32(VERTICAL_DIVISIONS);
		break;
